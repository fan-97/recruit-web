  ...
  <div>
    ...
    <el-dialog title="我的简历" :visible.sync="resumeFormVisible" class="myDialog">
      ...
      <el-form :model="resumeList" status-icon :rules="resumerules" ref="resumeInfo" label-width="100px"
        ...
        <el-input v-if="false" v-model="resumeList.id" auto-complete="off" class="hidden">{{this.resumeList.id}}</el-input>
  ...
  methods: {
    ...
    changeResume (formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.resumeList.skills = JSON.stringify(this.resumeList.skills);
          ...
          if (res.status === 200) {
            if (res.data.code === '000000') {
              if (res.data.data) {
                ...
                this.resumeList.skills = JSON.parse (this.resumeList.skills)
                this.len = this.resumeList.skills.length
              } else {
