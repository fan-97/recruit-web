...
<div class="wrapper-card">
  <div class="card" v-for="(item, key) in companyList" :key="key">
    <img :src="getImage(item.avatar)" class="image" @click="getCompanyDetail(item.id)">
</div>
</div>
  </div>
  <!--推荐-->
  ...
  <div class="newsContain">
    ...
    <div class="newsItem"  v-for = "(item, key) in jobList" :key = "key" @click="jobDetail(item.id)">
      <div class="picContain" ontouchstart="this.classList.toggle('hover');">
        <div class="flipper">
      <span class="itemPic el-icon-ice-cream-round"></span>
      <span class="back">￥{{item.treatment}}</span>
        </div>
    </div>
    <div>
      <p>{{item.name}}</p>
      <p style="margin-top:25px">{{item.introduce}}</p>
      </div>
    </div>
    </div>
  </div>
  ...
  <div class="aboutus">
    <div id="aboutusInfo">
    <h2>关于我们</h2>
    <p>因为专业所以更能明白您想要的，提供更多的实习和工作机会</p>
  ...
  <div class="footer">
    ...
    <a href="https://github.com/stalary"><img src="../assets/qq.gif"><span>https://github.com/stalary</span></a>
  </div>
</div>
</template>
<style>
...
.aboutus  {
  width: 100%;
  height: 500px;
  background: url("../assets/9381131-fd2872c9d933b2de.jpg") no-repeat;
...
export default {
  data () {
    return {
      ...
      jobList: [],
      recommandList: [],
      isHr: localStorage.getItem('role') === '2',
      isLogin: localStorage.getItem('token_expire') ? !localStorage.getItem('token_expire')<new Date().getTime : false,
      isShow:  false,
      getResumeList: {
          name: '',
          sex: '',
